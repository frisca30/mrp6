...
CORRECT_PASSWORD_HASH="a522ea1c5806e454ca5821fbb893c492"

echo "coba tebak passnya aph:"
read -s PASSWORD_INPUT
PASSWORD_INPUT_HASH=$(echo -n "$PASSWORD_INPUT" | md5sum | awk '{print $1}')
if [ "$PASSWORD_INPUT_HASH" != "$CORRECT_PASSWORD_HASH" ]; then
    echo "salah mek. Exiting."
    exit 1
fi
...
echo "Installing dependencies tunggu bntr mek..."
...
read -p "masukin private key eluwh: " PRIVATE_KEY
...
async function main() {
  ...
  const upgradedSwisstronik = await upgrades.deployProxy(Swisstronik, ['Hello Swisstronik from dakhil army !!'], { kind: 'transparent' });
...
async function main() {
  ...
  const messageToSet = "Hello Swisstronik from dakhil army!!";
